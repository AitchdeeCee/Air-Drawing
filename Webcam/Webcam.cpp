...
void trackObject(Mat imgThresh) {
    vector<vector<Point> > contours;  //hold the pointer to a contour
    CvSeq* result;     //hold sequence of points of a contour
    CvMemStorage* storage = cvCreateMemStorage(0); //storage area for all contours
    vector<Vec4i> hierarchy;

    //finding all contours in the image
    findContours(imgThresh, contours, hierarchy, CV_RETR_LIST, CV_CHAIN_APPROX_SIMPLE, cvPoint(0, 0));

    //iterating through each contour
    for( int i = 0; i< contours.size(); i++ )
    {
        //obtain a sequence of points of the countour, pointed by the variable 'countour'
        result = approxPolyDP(contours, sizeof(CvContour), storage, CV_POLY_APPROX_DP, cvContourPerimeter(contour) * 0.02, 0);
